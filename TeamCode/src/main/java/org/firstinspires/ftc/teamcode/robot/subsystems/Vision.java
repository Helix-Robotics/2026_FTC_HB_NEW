...
public class Vision {
    ...
    public double getFilteredTagX(int... desiredIds) {
        if (latestResult == null || !latestResult.isValid()) {
            return -181;
        }

        List<LLResultTypes.FiducialResult> detectedTags =
        ...
        for (int desiredId : desiredIds) {
            for (LLResultTypes.FiducialResult detectedTag : detectedTags) {
                if (detectedTag.getFiducialId() == desiredId) {
                    return detectedTag.getTargetXDegrees();
                }
            }
        }
    ...
    public class CheckForRedSideTag implements Action {
        ...
        public boolean run(@NonNull TelemetryPacket packet) {
            ...
            double tagX = getFilteredTagX(31, 32, 30, 33, 34, 35, 36, 37);
    ...
    public Action checkForRedSideTag() {return new CheckForRedSideTag(); }




    public class CheckForBlueSideTag implements Action {
        ...
        public boolean run(@NonNull TelemetryPacket packet) {
            ...
            double tagX = getFilteredTagX(39, 40, 41, 38, 42, 43, 44, 45);
    ...
    public Action checkForBlueSideTag() {return new CheckForBlueSideTag(); }
